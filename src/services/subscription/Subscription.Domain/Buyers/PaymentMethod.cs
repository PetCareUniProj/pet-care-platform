...
public class PaymentMethod : Entity
{
    ...
    public static Result<PaymentMethod> Create(int cardTypeId, string alias, string cardNumber, string securityNumber, string cardHolderName, DateTime expiration)
    {
        ...
        {
            return Result.Failure<PaymentMethod>(SubscriptionErrors.PaymentMethod.NullCardNumber);
        }

        if (string.IsNullOrWhiteSpace(securityNumber))
        {
            return Result.Failure<PaymentMethod>(SubscriptionErrors.PaymentMethod.NullSecurityNumber);
        }

        if (string.IsNullOrWhiteSpace(cardHolderName))
        {
            return Result.Failure<PaymentMethod>(SubscriptionErrors.PaymentMethod.NullCardHolderName);
        }

        if (expiration < DateTime.UtcNow)
        {
            return Result.Failure<PaymentMethod>(SubscriptionErrors.PaymentMethod.Expired);
        }
